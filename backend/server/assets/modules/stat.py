...
def get_stat(sessionId, start=None, finish=None):
    """
    Главный агрегатор статистики за период.
    Все обращения к БД выполняются внутри одной управляемой сессии.
    """
    if start is None:
    ...
    with Session() as db:
        ...
        canceled_orders_count = db.scalars(
            select(func.count(Orders.id)).where(
                and_(Orders.status == 'canceled'),
    ...
    # get_cashout сам откроет/закроет сессию внутри надеюсь =(
...
def get_expenses_stat(start, finish, db):
    expenses_data = (
        db.query(Expenses)
        .filter(and_(Expenses.date >= start), and_(Expenses.date <= finish))
        .all()
    )
    return [
        {
...
def get_supply_stat(start, finish, db):
    data = (
        ...
        .filter(Supplies.add_date >= start, Supplies.add_date <= finish)
        .group_by(Supplies.item_id)
        .all()
    )
    return [
        {
...
def get_writeoff_stat(start, finish, db):
    result = (
        ...
        .join(Storefront, WriteOff.item_id == Storefront.id)
        .filter(WriteOff.wo_date >= start, WriteOff.wo_date <= finish)
        .group_by(WriteOff.type)
        .all()
    )
...
def get_popular_prices():
    """
    Топ популярных цен за последнюю неделю.
    """
    with Session() as db:
        week = (datetime.now() - timedelta(days=7)).strftime(DATE_FORMAT_DEFAULT)
        rows = (
            db.query(func.count().label('count'), Orders.price)
...
def add_expense(amount, reason, user_uuid):
    """
    Добавление расхода + изменение кассы.
    """
    with Session() as db:
    ...
    # Корректировка кассы — отдельным вызовом (сама функция должна открыть сессию)
